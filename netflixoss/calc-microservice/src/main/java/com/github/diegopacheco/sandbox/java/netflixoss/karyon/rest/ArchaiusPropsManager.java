	...
	public ArchaiusPropsManager() {
		try {
			...
			prop.addCallback(new Runnable() {
				@Override
				public void run() {
					logger.info("polledCnfgiration: OMG my property just changed: " + DynamicPropertyFactory.getInstance().getStringProperty("netflixoss.archaius.props.createdBy", ""));
				}
			});
			logger.info("polledCnfgiration: My Prop is:" + prop.get());
